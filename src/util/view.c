...
void View_copy(Arena *arena, View *view, const String *copyable) {
   ...
   Error_assertnl(view->base != NULL, "View:View_copy - arena ran out of available memory, size is %lu, but tried to write to %lu.\n", arena->size, arena->used + view->size);
}
...
void View_copyCstring(Arena *arena, View *view, const char *copyable) {
   ...
   Error_assertnl(view->base != NULL, "View:View_copyCstring - arena ran out of available memory, size is %lu, but tried to write to %lu.\n", arena->size, arena->used + view->size);
}

// Get functions

char View_at(const View *view, size_t index) {
   Error_assertnl(index < view->size, "View:View_at - index %lu is out of bounds (view size is %lu).\n", index, view->size);
   return view->base[index];
}

char View_last(const View *view) {
   Error_assertnl(view->size != 0, "View:View_last - view is empty.\n");
   return view->base[view->size - 1];
}

char View_first(const View *view) {
   Error_assertnl(view->size != 0, "View:View_first - view is empty.\n");
   return view->base[0];
}
